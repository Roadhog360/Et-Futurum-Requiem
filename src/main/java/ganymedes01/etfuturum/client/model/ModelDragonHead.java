...
public class ModelDragonHead extends ModelBase
{
    ...
    public void render(Entity entityIn, float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scale)
    {
        ...
        GL11.glTranslatef(0.0F, -0.374375F, 0.0F);
        GL11.glScalef(0.75F, 0.75F, 0.75F);
        this.head.render(scale);
    }

    public void render(float rotationX, float rotationY, float animateTicks) {
        this.jaw.rotateAngleX = (float)(Math.sin((double)(animateTicks * (float)Math.PI * 0.2F)) + 1.0D) * 0.2F;
        this.head.rotateAngleY = rotationY / (180F / (float) Math.PI);
        this.head.rotateAngleX = rotationX / (180F / (float) Math.PI);
        GL11.glTranslatef(0.0F, -0.374375F, 0.0F);
        GL11.glScalef(0.75F, 0.75F, 0.75F);
